...
class Employee extends BaseController
{
    ...
    function emp_order_history()
    {
        $last = $this->uri->total_segments();
        $record_num = $this->uri->segment($last);
        if(is_numeric($record_num))
        {
            //echo "<pre>";print_r($record_num);die;
            $this->global['pageTitle'] = APP_NAME.' : Employee Order History';
            $data['history']=$this->employee_model->emp_order_history($record_num);
            $this->loadViews("emp_order_history_view", $this->global, $data, NULL);
        }
        else
        {
            redirect(base_url().'employee_listing');
        }
    }  
