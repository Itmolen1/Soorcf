...
class Sales_quotation extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('sales_quotation_model');
        $this->isLoggedIn();
    ...
    {
        $this->global['pageTitle'] = APP_NAME.' : Sales Quotation Listing';
        $this->loadViews("dashboard", $this->global, NULL , NULL);
    }

    function sales_quotation_listing()
    {        
        $searchText = $this->security->xss_clean($this->input->post('searchText'));
        $data['searchText'] = $searchText;
        $this->load->library('pagination');
        $count = $this->sales_quotation_model->sales_quotation_listing_count($searchText);
		$returns = $this->paginationCompress("sales_quotation_listing/", $count, 10 );
        $data['sales_quotation'] = $this->sales_quotation_model->sales_quotation_listing($searchText, $returns["page"], $returns["segment"]);
        //echo "<pre>";print_r($data);die;
        $this->global['pageTitle'] = APP_NAME.' : Sales Quotation Listing';
        $this->loadViews("sales_quotation_list_view", $this->global, $data, NULL);        
    }  
    ...
    function add_new_purchase_order()
    {
        if($this->input->post())
        {
            ...
            $poid = $this->sales_quotation_model->add_new_purchase_order($value,$sid);
            $all_session=$this->sales_quotation_model->get_all_by_session_id($sid);
            $this->sales_quotation_model->insert_session_to_boi($all_session,$poid);
            /*1.get all records from session table and add all the data in boi table */
        ...
        else
        {
            ...
            $data['vendor']=$this->sales_quotation_model->get_vendor_list();
            $data['item']=$this->sales_quotation_model->get_item_list();
            $data['unit']=$this->sales_quotation_model->get_unit_list();
            $data['bill_no']=$this->sales_quotation_model->get_bill_no();
    ...
    function add_po_boi_session()
    {
        ...
        {
            $value=$this->input->post();
            $result=$this->sales_quotation_model->add_po_boi_session($value);
    ...
    function delete_po_boi_session()
    {
        if($this->input->post())
        {
            ...
            $get_session=$this->sales_quotation_model->get_po_boi_po_id_session($record_num['data']);
            $result=$this->sales_quotation_model->delete_po_boi_session($record_num['data']);
    ...
    {
        $result=$this->sales_quotation_model->get_items_by_session_id($po_boi_po_id_session);
        $finalresult='';
        $subtotal=0.0;
        $taxamount=0.0;
        $grandtotal=0.0;
    ...
    function edit_purchase_order()
    {
        if($this->input->post())
        {
        	...
        	$sid=$this->session->userdata['purchase_order_item']['po_boi_po_id_session'];
            $all_session=$this->sales_quotation_model->get_all_by_session_id($sid);
            $this->sales_quotation_model->insert_session_to_boi($all_session,$value['purchase_order_id']);
            $result = $this->sales_quotation_model->update_purchase_order($value);
            if(isset($this->session->userdata['purchase_order_item']['po_boi_po_id_session']))
            {
            	$this->session->unset_userdata('purchase_order_item');
            }
        ...
        else
        {
            ...
            {
                $data['purchase_order'] = $this->sales_quotation_model->get_po_by_id($record_num);
            }
            $data['boi']=$this->sales_quotation_model->get_all_boi_by_poid($record_num);
            if(empty($data['boi']))
            {
            	...
            	//$res=$this->sales_quotation_model->inset_boi_to_session($data['boi']);
    ...
    function delete_purchase_order()
    {
        $last = $this->uri->total_segments();
        $record_num = $this->uri->segment($last);
        $result = $this->sales_quotation_model->delete_purchase_order($record_num);
        redirect('purchase_order_listing','refresh');        
    }
    ...
    {
    	$value=$this->input->post();
    	$result=$this->sales_quotation_model->purchase_order_get_payment_record_details($value['data']);
    ...
    {
    	$value=$this->input->post();
    	$this->sales_quotation_model->purchase_order_add_payment_record($value);
    	redirect('purchase_order_listing','refresh');
    ...
    function purchase_order_email()
    {
        ...
        $data = $this->sales_quotation_model->get_po_pdf_data($value['data']);
        //recipient
    ...
    public function purchase_order_pdf()
    {
        ...
        $value=$this->input->post();
        $data = $this->sales_quotation_model->get_po_pdf_data($value['data']);
        $boi =  $this->sales_quotation_model->get_all_boi_by_poid_pdf($data['purchase_order_id']);
        //echo "<pre>";print_r($data);die;
    ...
    public function po_export_exl()
    {
    	$last = $this->uri->total_segments();
		$record_num = $this->uri->segment($last);
    	$data = $this->sales_quotation_model->get_po_pdf_data($record_num);
    	//echo "<pre>";print_r($data);die;
