<?php
	...
	class User_api_model extends CI_Model 
	{
		...
		function get_user_address_list($data)
		{
			...
			$sql="SELECT user_address_id,user_address,user_latitude,user_longitude,person_name,person_mobile FROM tbl_user_address  WHERE isDeleted = 0 AND user_id = ".$data['user_id']." ORDER BY user_address_id DESC ";
			$query = $this->db->query($sql);				
			$rows=$query->result_array();
			return $rows;
		}
