...
class Contact extends CI_Controller {
	...
	public function add()
	{
		...
		if(isset($input['item_master_name']))
		{
			$input['item_master_name']=$input['item_master_name'];
		}
		else
		{
			$input['item_master_name']='N.A.';
		}
		//echo "<pre>";print_r($input);die;
		$val=array('subject'=>$input['subject'],'name'=>$input['name'],'email'=>htmlentities($input['email']),'mobile_number'=>$input['mobile_number'],'comments'=>htmlentities($input['comments']),'created_at'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s'),'status'=>'0','item_master_name'=>htmlentities($input['item_master_name']));
		$result=$this->contact_model->add($val);
		$this->session->set_userdata('contact_message','Your Query Successfully Recorded, we will contact you as soon as possible.');
